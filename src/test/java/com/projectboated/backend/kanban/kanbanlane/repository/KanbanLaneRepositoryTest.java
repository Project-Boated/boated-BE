...
class KanbanLaneRepositoryTest extends RepositoryTest {
	...
	@Test
	void findByProjectIdAndName_project에속한이름이존재한KanbanLane이1개존재_1개조회(){
	    // Given
		Project project = insertProjectAndCaptain(USERNAME, NICKNAME);
		Kanban kanban = insertKanban(project);
		KanbanLane kanbanLane = insertKanbanLanes(project, kanban, 4).get(0);

		// When
		Optional<KanbanLane> result = kanbanLaneRepository.findByProjectIdAndName(project.getId(), kanbanLane.getName());

		// Then
	    assertThat(result).isPresent();
		assertThat(result.get().getId()).isEqualTo(kanbanLane.getId());
	}
